...
2. **Manual ExtendScript workflow** - adobe-mcp not configured, using manual Premiere scripts
...
The project already contains a custom Premiere Pro MCP integration at `mcp/adobe-mcp/`:
...
1. Continue using the existing `mcp/adobe-mcp` local integration
...
|------|-------------|-------------|
| `mcp/adobe-mcp/uxp-plugins/premiere/commands/index.js` | +400 | 25+ new command handlers |
| `mcp/adobe-mcp/jsx_runner.py` | +250 | Python wrapper functions |
| `mcp/adobe-mcp/premiere_tools.py` | +300 | MCP tool decorators |
