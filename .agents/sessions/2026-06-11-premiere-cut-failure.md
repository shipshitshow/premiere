...
- Verify after every cut batch with `get_full_project_data`, not only tool success.
...
- If gaps remain after cutting, either:
  - use the documented Premiere gap-closing workflow if it is known to be safe, or
  - stop and tell the user the cuts are done and gaps require manual packing.
